...
namespace ConsoleApp1
{
...

    {
        ...
        static void Main(string[] args)
        {
			...
			//Removing previous files to prevent conflicts and unncessacarily long files
            ...
            Thread.Sleep(100);
            bool isInternetConnected = System.Net.NetworkInformation.NetworkInterface.GetIsNetworkAvailable(); //Checks if internet is connected
            Console.WriteLine("Is Internet Connected / Are connections available :     {0}", isInternetConnected); //Writes if internet is connected

            if (String.ReferenceEquals(isInternetConnected, False)) //If internet isn't connected
            {
            ...
            string fullURLuser = Console.ReadLine(); //Stores the input as a string
            System.IO.File.WriteAllText("Playlist.txt", fullURLuser); //Writes the URL to a file

            string isYouTube = fullURLuser.Remove(fullURLuser.Length - 34); //Removes the playlistID
